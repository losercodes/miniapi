...
class MiniAPI {
    constructor(options = {}) {
      ...
      // Set up automatic logging if enabled
      if (this.options.logging) {
        this.use(async (ctx, next) => {
          ...
          ctx.res.end = function(...args) {
            ...
            console.log(`[${new Date().toISOString()}] ← ${ctx.req.method} ${ctx.req.url} (${ctx.res.statusCode}) - ${duration}ms`);
    ...
    async listen(port = 3000, callback) {
      ...
      // Add default root route if none exists
      if (!this.routes['GET:/']) {
        this.get('/', (ctx) => {
      ...
      const server = http.createServer(async (req, res) => {
        ...
        try {
          ...
          // If middleware chain was interrupted, don't proceed to route handler
  ...
  // Fix: Properly handle the exports based on environment
  // For ESM (ES modules)
  export { MiniAPI, createAPI };
  export default createAPI;
  
  // For CommonJS - Use conditional exports instead of directly accessing 'module'
